...
<body background="\images\bg_html.jpg">
			...
			<img src="http://blog.asiayo.com/wp-content/uploads/2016/10/%E5%8D%97%E6%8A%95-1920x768.jpg" class="top_pic">
				...
				<img src="http://pic.pimg.tw/stephytruth/1309079219-c7c102fc171192181df4f58b06e14deb.png" alt="" class="tuding2">	
 						         ...
 						         <form action="synopsis_s.php" method="POST">
 						         	<label>帳號</label>
											<?php 
...
<?php
    		...
    		require_once("db_config.php");
    		if (!isset($_SESSION['U_ID'])) {
				$get_sql = "SELECT travel.* from travel where travel.travel_type ='南部' order by travel_id";
		...
		<?php } 
					...
					$rs=mysqli_fetch_row($data);
					$get_sql = "SELECT travel.*, favorite.favarite_id, favorite.member_id from travel left join favorite on travel.travel_id = favorite.travel_id and favorite.member_id = $mem_id where travel.travel_type ='南部' order by travel_id";
    				$data = mysqli_query($link, $get_sql);
					for($i=1;$i<=mysqli_num_rows($data);$i++){ 
					$rs=mysqli_fetch_row($data);
		?>
